 ...
 * 
 */
public class MscShellExecutor implements ShellExecutor {

	private static final Logger logger = Logger.getLogger(MscShellExecutor.class);

	private MscManagement mscManagement;
	private MscPropertiesManagement mscPropertiesManagement = MscPropertiesManagement.getInstance();

	/**
	 * 
	 */
	public MscShellExecutor() {
		// TODO Auto-generated constructor stub
	}

	public MscManagement getMscManagement() {
		return mscManagement;
	}

	public void setMscManagement(MscManagement mscManagement) {
	...
	public String execute(String[] commands) {
		...
		try {
			if (commands.length < 2) {
				return MscOAMMessages.INVALID_COMMAND;
			}
	...
	private String manageSet(String[] options) throws Exception {
		...
		if (parName.equals(MscPropertiesManagement.MSC_GT)) {
			mscPropertiesManagement.setMscGt(options[3]);
		} else if (parName.equals(MscPropertiesManagement.GMLC_SSN)) {
			int val = Integer.parseInt(options[3]);
			mscPropertiesManagement.setGmlcSsn(val);
		} else if (parName.equals(MscPropertiesManagement.HLR_SSN)) {
			int val = Integer.parseInt(options[3]);
			mscPropertiesManagement.setHlrSsn(val);
		} else if (parName.equals(MscPropertiesManagement.MSC_SSN)) {
			int val = Integer.parseInt(options[3]);
			mscPropertiesManagement.setMscSsn(val);
		} else if (parName.equals(MscPropertiesManagement.MAX_MAP_VERSION)) {
			int val = Integer.parseInt(options[3]);
		...
		return MscOAMMessages.PARAMETER_SUCCESSFULLY_SET;
	}
	...
	private String manageGet(String[] options) throws Exception {
		if (options.length == 3) {
			...
			StringBuilder sb = new StringBuilder();
			sb.append(MscPropertiesManagement.MSC_GT+" = ");
			sb.append(mscPropertiesManagement.getMscGt());
			sb.append("\n");

			sb.append(MscPropertiesManagement.GMLC_SSN+ " = ");
			sb.append(mscPropertiesManagement.getGmlcSsn());
			sb.append("\n");

			sb.append(MscPropertiesManagement.HLR_SSN+" = ");
			sb.append(mscPropertiesManagement.getHlrSsn());
			sb.append("\n");

			sb.append(MscPropertiesManagement.MSC_SSN +" = ");
			sb.append(mscPropertiesManagement.getMscSsn());
			sb.append("\n");

			sb.append(MscPropertiesManagement.MAX_MAP_VERSION+ " = ");
			sb.append(mscPropertiesManagement.getMaxMapVersion());
			sb.append("\n");
